...
* Para calcular $P(B \mid A)$: a probabilidade de Bob receber $1$ se Alice enviar $1$ é a probabilidade de não haver erro quando Alice envia $1$: 
  ...
  Ou, se quisermos ser mais formais, usando a definição:
  
  $$
  \begin{aligned}
  P(B \mid A) &= \frac{P(A \cap B)}{P(A)} \\
  &= \frac{1/2 \cdot 90/100}{1/2} \\
  &= \frac{90}{100}
  \end{aligned}
  $$
...
* Então, $P(B \mid A) \cdot P(A) = \frac{90}{100} \cdot \frac12 = \frac{9}{20}$.

* Para calcular $P(B)$, usamos probabilidade total, condicionando sobre $A$:

  $$
  P(B) = P(B \mid A) \cdot P(A) \;+\;
  P(B \mid \neg A) \cdot P(\neg A)
  $$

* A primeira parcela foi calculada acima. A segunda parcela é
  ...
  \frac{P(\neg A \cap B)}{P(\neg A)} \cdot \frac12 \\
  &=
  \frac{1/2 \cdot 5/100}{1/2} \cdot \frac12 \\
  &=
...
* Daí, $P(B) = \frac{9}{20} + \frac{1}{40} = \frac{19}{40}$
...
* Queremos calcular $P(AAA \mid BB\neg B)$, onde $AAA$ é o evento "Alice enviou $111$" e $BB\neg B$ é o evento "Bob recebeu $110$".

* Como os erros são independentes para cada *bit*, basta calcular

  $$
  P(A \mid B) \cdot P(A \mid B) \cdot P(A \mid \neg B)
  $$

  com a notação do item (a).
  
* Usando Bayes:

  $$
  P(A \mid \neg B) = \frac{P(\neg B \mid A) \cdot P(A)}{P(B)}
  $$
...
* Concluímos que $P(A \mid \neg B) = 2/21$.

* A probabilidade procurada é

  $$
  P(AAA \mid BB\neg B) = \left(\frac{18}{19}\right)^2 \cdot \frac{2}{21} =
  \frac{216}{2527} \approx `r (216/2527) %>% fm(6)`
  $$
...
* No esquema [sem repetição]{.hl}, a probabilidade de Bob receber $1$ quando Alice transmitiu $1$ era $0{,}9$.

* No esquema [com repetição]{.hl}, a probabilidade de Bob receber uma das $4$ cadeias quando Alice transmitiu $111$ é

  $$
  \frac{9^3}{10^3} +
  \frac{9^2}{10^3} \cdot 3 
  = \frac{243}{250}
  \approx 
  `r ((3 * 9^2 + 9^3) / 10^3) %>% fm(6)`
  $$

* No esquema [sem repetição]{.hl}, a probabilidade de Alice ter enviado $1$ quando Bob recebeu $1$ (calculada no item (a)) era $0{,}95$.

* No esquema [com repetição]{.hl}, a probabilidade de Alice ter enviado $111$ quando Bob recebeu alguma das cadeias é ???
