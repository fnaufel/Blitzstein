...
* [Convenientemente, $|D|$ desapareceu, mas só porque $p = 1/2$.]{.hl}
...
* Vamos simular a situação. Definimos um conjunto $A$ fixo, com $p = 1/2$, e geramos diversos conjuntos $D$, testando se $A = D$:

    ```{r}
    # Simular um conjunto D e testar se D = A
    simular_1 <- function(
      A,                # Conjunto A (fixo)
      tamanho_C         # qtde de pessoas em C
    ) {
      
      # O tamanho de D não é necessariamente igual ao de A
      # Sortear o tamanho de D
      tamanho_D <- sample(tamanho_C, 1)
      
      if (tamanho_D == length(A)) {
        
        # Se os tamanhos coincidirem, gerar D e testar
        D <- sort(sample(tamanho_C, tamanho_D))
        # Como A e D estão ordenados, basta isto:
        all(A == D) 
    ...
    resultados <- 1:n %>% 
      map_lgl(
        ~simular_1(A, tamanho_C)
      )
    
    cat('\nP(A = D) simulado = ', mean(resultados))
    cat('\nP(A = D) teórico  = ', 1/2^tamanho_C)
    ```
...
```{r}
simular_2 <- function(p, n, nsubconjuntos) {
  
  A <- (1:n)[runif(n) <= p]
  
  tamanho_D <- sample(0:n, size = 1, prob = nsubconjuntos)
  D <- sort(sample(n, tamanho_D))
...
p <-  1/2
n <- 10
nsubconjuntos <- choose(n, 0:n)
nsims <- 1e5

mean(
  1:nsims %>% 
    map_lgl(~simular_2(p, n, nsubconjuntos))
)
```
